...
public:
	...
	pair<Mat, Mat> initialize_params()
	{
		...
		covarMat = covarMat*(rand() % 255) * 1000;
	...
	pair <Mat, Mat> Estep(vector<vector<int> >dataset,
		...
		for (int i=0; i<dataset.size(); i++){
			...
			Mat d = (single_dataset - mean)*(covar.inv());
			Mat d1 = d*((single_dataset - mean).t());
			expectation_h.at<double>(i,0) = (v+D)/(v+d1.at<double>(i,0));
			expectation_logh.at<double>(i,0) = boost::math::digamma(((v/2) + (D/2)))
											- exp((v/2)+ (d1.at<double>(i,0)/2));
...
//			expectation_logh[i] = scipy.special.digamma(((v/2) + (D/2)), out=None) - np.log((v/2) + (d1[i]/2))
	...
	pair<Mat, Mat> Mstep(vector<vector<int> >dataset,
		...
		for (int i=0; i<dataset.at(0).size(); i++){
			...
			new_mean.at<double>(0,i) = a.at(0)/y_sum;
		}

		cv::Mat covar_temp( dataset.at(0).size(),dataset.size(), CV_64FC1);
		for (int i=0; i<dataset.at(0).size(); i++){
			double arr[dataset[i].size()];
			std::copy(dataset[i].begin(), dataset[i].end(), arr);
			cv::Mat single_dataset(1, 100, CV_64FC1, arr);
			single_dataset = single_dataset.reshape(1,1);
			Mat d = ((single_dataset - new_mean).t())* (expectation_h.at<double>(i,0)*(single_dataset - new_mean));
			covar_temp.row(i) = d.reshape(1,1);
		}
		cv::Mat new_covar(1, dataset.at(0).size(), CV_64FC1);
		for (int i=0; i<dataset.at(0).size(); i++){
			vector<double> a;
			cv::reduce(covar_temp(Range(i,i+1), Range::all()), a,0,CV_REDUCE_SUM);
			new_covar.at<double>(0,i) = a.at(0)/y_sum;
		}
...
//	    new_v = final[0]
	...
	void train(vector<vector<int> > train_face,
			...
			for (int i=0; i<n_iterations; i++){
				...
				Mat expectation_h_face = estep_face.first;
				Mat expectation_logh_face = estep_face.second;
				pair<Mat, Mat> mstep_face = Mstep(train_face,
												expectation_h_face,
												expectation_logh_face,
												v_face);
				Mat mean_face = mstep_face.first;
				Mat covar_face = mstep_face.second;
				v_face= cal_v(v_face,
							expectation_h_face,
							expectation_logh_face);
				...
				Mat expectation_h_nonface = estep_nonface.first;
				Mat expectation_logh_nonface = estep_nonface.second;
				pair<Mat, Mat> mstep_nonface = Mstep(train_nonface,
												expectation_h_nonface,
												expectation_logh_nonface,
												v_nonface);
				Mat mean_nonface = mstep_nonface.first;
				Mat covar_nonface = mstep_nonface.second;
